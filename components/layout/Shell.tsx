...
import {
  Inbox, Ticket, FolderKanban, FileText, Users, ScrollText,
  LogOut, ChevronRight, ClipboardList, Milestone, CreditCard,
  UserCircle2,
} from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-[#0d1117] overflow-hidden">
      ...
      <aside className="w-52 flex-shrink-0 bg-[#161b22] border-r border-white/[0.08] flex flex-col">
        ...
        {/* Brand */}
        <div className="px-4 h-12 flex items-center gap-2 border-b border-white/[0.08] flex-shrink-0">
          ...
          {nav.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <Link key={href} href={href}
                ...
                <Icon
                  size={14}
                  className={active ? 'text-[#f0883e]' : 'opacity-60 group-hover:opacity-100 transition-opacity'}
                />
                {label}
                {active && <ChevronRight size={11} className="ml-auto text-[#484f58]" />}
              </Link>
            );
          })}
        </nav>
        ...
        {/* Profile link */}
        <Link href="/profile"
          className={`flex items-center gap-2.5 px-3 py-2 mx-2 mb-1 mt-1 rounded-md text-[13px] transition-colors group ${
            pathname === '/profile'
              ? 'bg-white/[0.06] text-[#e6edf3] font-medium'
              : 'text-[#8b949e] hover:bg-white/[0.04] hover:text-[#e6edf3]'
          }`}>
          <UserCircle2 size={14} className={pathname === '/profile' ? 'text-[#f0883e]' : 'opacity-60 group-hover:opacity-100 transition-opacity'} />
          Profile
        </Link>
        ...
        <div className="p-3 border-t border-white/[0.08] flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className={`inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-mono font-bold uppercase tracking-wider ${ROLE_BADGE[user.role] ?? 'bg-white/5 text-[#484f58]'}`}>
                {user.role}
              </div>
            </div>
            <form action={logoutAction}>
              <button type="submit" title="Sign out"
                className="w-7 h-7 rounded-md border border-white/[0.08] flex items-center justify-center text-[#484f58] hover:text-red-400 hover:border-red-400/30 hover:bg-red-500/10 transition-colors flex-shrink-0">
                <LogOut size={12} />
              </button>
            </form>
          </div>
